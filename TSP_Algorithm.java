...
public class TSP_Algorithm {
	...
	private Path TDOptimalPath; //Time dependent optimal path
	...
	private void calcTimeDependentOptimalPath(){
		ArrayList<ArrayList<Vertex>> treeList = tree.getAllCombinations();
		pathsList = new ArrayList<Path>(treeList.size());
		...
		// Base case path
		TDOptimalPath = pathsList.get(0);

		for (Path p : pathsList){
			if (p.getTDDist() < TDOptimalPath.getTDDist()) {
				TDOptimalPath = p;
			}
		}
	}

	public Path getTDOptimalPath() {return TDOptimalPath; }
