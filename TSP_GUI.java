...
public class TSP_GUI extends JFrame implements ActionListener {
	...
	private final JLabel tspLabel;
	private final JLabel intro;
	private final JLabel intro2;
	private final CityMapPanel cityMap;
	private static JTextArea cityInfo;
	private final JLabel oLabel; // Optimal Button instructions
	private final JLabel oLabel2;
	private final JLabel nLabel; // Nearest Neighbor Button instructions
	private final JLabel nLabel2;
	private final JButton oButton; // Optimal Button
	private final JButton nButton; // Nearest Button
	private final JTextField oField;
	private final JTextField nField;

	private static Vertex[] cities;
	private static ArrayList<Edge> edges;
	private final TSP_Algorithm algo;
	...
	public TSP_GUI(int n) throws UnderflowException, FileNotFoundException {
		super("Time dependent Traveling Salesman Problem");
		setLayout(null);
		...
		tspLabel = new JLabel("TDTSP");
		float fontSize = 20;
		tspLabel.setFont(tspLabel.getFont().deriveFont(fontSize));
		
		intro = new JLabel("2 hành trình tối ưu: không và có bị ảnh hưởng bởi thời gian");
		intro2 = new JLabel("Chi phí: 1 đồng/ 1 km");
		...
		oLabel = new JLabel("Người bán hàng cần tìm đường đi ngắn nhất");
		oLabel2 = new JLabel("Tìm giải pháp cho anh ta:");
		nLabel = new JLabel ("Anh ta muốn biết chi phí thực tế do bị ảnh hưởng bởi thời gian");
		nLabel2 = new JLabel("Đưa ra kết quả cho người bán hàng:");

		oButton = new JButton ("Chi phí tối ưu");
		nButton = new JButton ("Chi phí phụ thuộc thời gian");

		oField = new JTextField("Chi phí gốc ... ");
		nField = new JTextField("Chi phí thực tế ... ");
	...
	public void getCities(int n) throws FileNotFoundException{
		Vertex v;
		double VX;
		double VY;
...
//			VX = (Math.random()+1)*(Math.random()+1)*400;
//			VY = (Math.random()+1)*(Math.random()+1)*400;
			VX = a[i][0];
			VY = a[i][1];
	...
	public void actionPerformed(ActionEvent ae){
		if (ae.getSource() == oButton) {
			...
			oField.setText("Chi phí tối ưu = " + oPath.getDist());
		}

		if (ae.getSource() == nButton){

				Path nPath = algo.getTDOptimalPath();
				cityMap.drawTDOptimalPath(nPath);
				//cityMap.drawNearPath(nPath);
				//nField.setText("Estimated Shortest Path = " + nPath.getDist());
				nField.setText("Chi phí tối ưu thực tế = " + nPath.getDist());
	...
	private static class CityMapPanel extends JPanel {
		...
		public void drawOptimalPath(Path p) {
			...
			// Paints nodes in shortest path blue.
		...
		// Draws the time dependent optimal path
		public void drawTDOptimalPath(Path p) {
			Graphics2D g = (Graphics2D) getGraphics();
			...
			// Paints nodes in shortest path red.
			for (Vertex v : p.getPath()){
				double vx = getX_coord(v);
				double vy = getY_coord(v);
